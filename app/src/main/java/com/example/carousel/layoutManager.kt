 ...
 */
class ScrollZoomLayoutManager(private val context: Context, itemSpace: Int) :
    ...
    private fun calculateScale(x: Int?): Float {
        val deltaX = x?.minus(horizontalSpace - mDecoratedChildWidth)?.div(2)?.let { abs(it) }
        var diff = 0f
    ...
    private fun computeScrollVectorForPosition(targetPosition: Int): PointF? {
        ...
        val firstChildPos = getChildAt(0)?.let { getPosition(it) }
        val direction = if (targetPosition < firstChildPos!!) -1 else 1
        return PointF(direction.toFloat(), 0.0f)
    }
